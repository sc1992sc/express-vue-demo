...
var dialog = {} // 定义插件对象
dialog.install = function (Vue, options) { // vue的install方法，用于定义vue插件
 // 如果toast还在，则不再执行
...
Vue.prototype.$alert = function(options){ // 在Vue的原型上添加实例方法，以全局调用
    ...
    $vm.visible = true // 显示toast
}
}
export default dialog; // 导出Toast（注意：此处不能用module exports导出，在一个文件中，不能同时使用require方式引入，而用module exports导出，两种方式不能混用）
